...
class LiquifyPreviewPanel(QtWidgets.QWidget):
    ...
    def __init__(self, state: str = "ready", width_mode: str = "standard", parent=None):
        ...
        self.blocker_details = QtWidgets.QFrame(content)
        self.blocker_details.setObjectName("RizumLiquifyBlockers")
        blocker_layout = QtWidgets.QVBoxLayout(self.blocker_details)
        blocker_layout.setContentsMargins(10, 8, 10, 8)
        blocker_layout.setSpacing(4)
        blocker_title = QtWidgets.QLabel(_text("blockers"), self.blocker_details)
        blocker_title.setObjectName("RizumLiquifyBlockerTitle")
        blocker_layout.addWidget(blocker_title)
        self._blocker_labels = []
        for key in ("blocker_1", "blocker_2"):
            label = QtWidgets.QLabel(_text(key), self.blocker_details)
            label.setObjectName("RizumLiquifyBlockerPath")
            label.setSizePolicy(
                QtWidgets.QSizePolicy.Policy.Ignored,
                QtWidgets.QSizePolicy.Policy.Preferred,
            )
            blocker_layout.addWidget(label)
            self._blocker_labels.append(label)
        self.blocker_details.setStyleSheet(
            """
...
"""
        ...
        self.primary_button = SecondaryActionButton(
            "",
    ...
    def _set_target_items(self, has_target: bool) -> None:
        blocker = QtCore.QSignalBlocker(self.target_combo)
        self.target_combo.clear()
        if has_target:
            self.target_combo.addItem(_text("target_folder"), "folder")
            self.target_combo.addItem(_text("target_set"), "set")
            self.target_combo.setEnabled(True)
        else:
            self.target_combo.addItem(_text("target_none"), None)
    ...
    def setState(self, state: str, *, emit: bool = True) -> None:
        ...
        self._set_target_items(has_target)
        self._blockers_expanded = False
        self.blocker_details.hide()
...
def build_liquify_preview(QtWidgets_module=None):
    ...
    panel_host = QtWidgets.QWidget(page)
    panel_host.setObjectName("RizumTransparent")
    panel_layout = QtWidgets.QHBoxLayout(panel_host)
    panel_layout.setContentsMargins(0, 0, 0, 0)
    panel_layout.setSpacing(0)
    panel_layout.addStretch(1)
    panel = LiquifyPreviewPanel()
    panel_layout.addWidget(panel, 0, QtCore.Qt.AlignmentFlag.AlignTop)
    panel_layout.addStretch(1)
    page_layout.addWidget(panel_host, 1)

    def select_state(_index: int) -> None:
        panel.setState(str(state_control.currentData() or "ready"))

    def select_width(_index: int) -> None:
        panel.setWidthMode(str(width_control.currentData() or "standard"))
