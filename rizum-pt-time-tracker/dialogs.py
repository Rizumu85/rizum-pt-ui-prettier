...
from rizum_ui import (PainterSettingsDialog, PAINTER_SETTINGS_LAYOUT as METRICS,
                      ...
                      AnimatedSaveButton, build_stylesheet, default_theme,
...
class AssignmentDialog(TrackerDialog):
    def __init__(self, ledger, path, parent=None):
        super().__init__('Work and part', parent)
        self.ledger = ledger
        binding = ledger.binding(path)
    ...
    def validate(self, *_args):
        ...
        self.work_name_row.setVisible(new_work)
        self.part_name_row.setVisible(new_part)
        valid = (not new_work or bool(self.work_name.text().strip())) and (not new_part or bool(self.part_name.text().strip()))
        changed = self.selection() != getattr(self, 'initial', None)
        self.save_button.setDirty(valid and changed, animate=False)


class NumberDialog(TrackerDialog):
    def __init__(self, title, label, value, minimum, maximum, parent=None, *, step=1):
        super().__init__(title, parent, action='Add' if title == 'Add time' else 'Save')
        self.number = make_compact_stepper(value, minimum, maximum, step=step)
        self.add_field(label, self.number)
        if title != 'Add time':
            self.save_button.setDirty(False, animate=False)
            self.number.valueChanged.connect(lambda current: self.save_button.setDirty(current != value, animate=False))
