...
- The collapsible group animation remains the most fragile area. Attempts tried so far:
  ...
  The current version uses a single fixed-height frame with child rows visible during the height animation. If this still jumps in Painter, consider a custom-painted cached pixmap for the child content during collapse/expand instead of live child widgets.
- Icon buttons were changed to render SVGs with `QSvgRenderer` at device pixel ratio because `QIcon.pixmap()` looked soft. Re-check in Painter before tuning sizes further.
