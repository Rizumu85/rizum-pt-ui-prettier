...
Create instances using the appropriate static methods depending on what you want to do:
...
- **`from_textureset_stack(stack)`** - Generate an InsertPosition on the top of a stack. Only a LayerNode can be inserted at the top of the stack.

- **`above_node(node)`** - Generate an InsertPosition to insert above the given node. Only a LayerNode can be inserted above a LayerNode. Only an EffectNode can be inserted above an EffectNode.

- **`below_node(node)`** - Generate an InsertPosition to insert below the given node. Same rules as above_node apply.

- **`inside_node(node, node_stack)`** - Generate an InsertPosition to insert inside the given stack of a node. Only a LayerNode can be inserted inside a GroupLayerNode if node_stack is NodeStack.Substack. Only an EffectNode can be inserted inside a LayerNode if node_stack is NodeStack.Content or NodeStack.Mask.
